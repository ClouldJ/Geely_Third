...
-(instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, ([self getHeightValueWith:self.title andFont:14] - 10)/2, 1.5, 10)];
        imageView.image = [UIImage imageNamed:@"Geely_Custom_btnprompt"];
        [self addSubview:imageView];
        
        
        self.label = [[UILabel alloc] initWithFrame:CGRectMake(6, 0, [self getWidthValueWith:self.title andFont:14], [self getHeightValueWith:self.title andFont:14])];
...
-(instancetype)initWithCustomType:(ButtonType)type andFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        [self loadSubViewsWith:type];
    }
    return self;
}

-(void)loadSubViewsWith:(ButtonType)type {
    ...
    switch (type) {
        case Left:
        {
            self.topView = [[GeelyCustomPromptView alloc] initWithFrame:CGRectMake(self.frame.size.width-[self getWidthValueWith:str andFont:14], 0, 100, 100)];
        }
            break;
        case Center:
        {
            self.topView = [[GeelyCustomPromptView alloc] initWithFrame:CGRectMake((self.frame.size.width - [self getWidthValueWith:str andFont:14]-6)/2, 0, 100, 100)];
        }
            break;
        case Right:
        {
...
//        NSArray *data = [NSArray arrayWithObjects:@"", nil]
        ...
        for (int i = 0; i<9; i++) {
            if (i%3 == 0) {
                customBtn = [[GeelyContentCustomButton alloc] initWithCustomType:Left andFrame:CGRectMake(85+(i%3)*30+(i%3)*w, (100+(i/3)*(50+50))*(2620/1920), w, 25)];
            }else if (i%3 == 1) {
                customBtn = [[GeelyContentCustomButton alloc] initWithCustomType:Center andFrame:CGRectMake(85+(i%3)*30+(i%3)*w, (100+(i/3)*(50+50))*(2620/1920), w, 25)];
            }else if (i%3 == 2) {
                customBtn = [[GeelyContentCustomButton alloc] initWithCustomType:Right andFrame:CGRectMake(85+(i%3)*30+(i%3)*w, (100+(i/3)*(50+50))*(2620/1920), w, 25)];
            }else{
